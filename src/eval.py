...
@utils.task_wrapper
def evaluate(_config: DictConfig) -> Tuple[dict, dict]:
    ...
    trainer: Trainer = Trainer(**_config.trainer, logger=logger)
    ...
    # predictions = trainer.predict(model=model, dataloaders=dataloaders, ckpt_path=_config.ckpt_path)
...
def main(_config: DictConfig) -> None:
    ...
    # (e.g. ask for tags if none are provided in _config, print _config tree, etc.)
